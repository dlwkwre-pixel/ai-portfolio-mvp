...
export async function rebuildLinkedPortfolioHistory(
  ...
  let returnMethod: string | null = brokerReturn != null ? "broker" : null;
  if (returnPct == null) {
    ...
    if (!isSplit && depositsClean) { returnPct = netDepositPct!; returnMethod = "net-deposit"; }
    else if (isSplit && isDefaultTarget && recon.pricedCoverage >= 0.85 && indexGrowthPct != null) { returnPct = indexGrowthPct; returnMethod = "growth-index"; }
    else if (isSplit && isDefaultTarget && recon.pricedCoverage >= 0.85 && recon.returnPct != null) { returnPct = recon.returnPct; returnMethod = "money-weighted"; }
    else if (positionsCost > 0) { returnPct = ((positionsValue - positionsCost) / positionsCost) * 100; returnMethod = "cost-basis"; }
    else if (depositsClean) { returnPct = netDepositPct!; returnMethod = "net-deposit"; }
  }
  ...
  await admin.from("chart_cache").upsert({
    ...
    result: useRecon
      ? {
          ...
          // Health metrics for the user-facing confidence tooltip — surfaces the checks
          // the sync already runs so a confused user sees WHY the number is trustworthy.
          health: {
            method: returnMethod,
            coverage: Math.round(recon.coverage * 100) / 100,
            pricedCoverage: Math.round(recon.pricedCoverage * 100) / 100,
            verifiedAt: new Date().toISOString(),
          },
        }
      : { symbol: benchSymbol, series: [], benchReturnPct: null, twrIndex: [], health: { method: returnMethod, coverage: Math.round(recon.coverage * 100) / 100, pricedCoverage: Math.round(recon.pricedCoverage * 100) / 100, verifiedAt: new Date().toISOString() } },
