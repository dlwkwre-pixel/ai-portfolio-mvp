...
import WindfallClient from "./windfall-client";

const HIGH_INTEREST_CATS = new Set(["credit_card", "credit card", "personal_loan", "personal loan", "other_debt", "debt", "medical_debt"]);
const LIQUID_CATEGORIES = new Set(["cash", "savings", "checking", "emergency_fund", "money_market"]);

export default async function WindfallPlanningPage() {
  ...
  const highInterestDebt = (balanceItems ?? [])
    .filter((i) => i.is_liability && Number(i.value) > 0 && HIGH_INTEREST_CATS.has((i.category ?? "").toLowerCase()))
