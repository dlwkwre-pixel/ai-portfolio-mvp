...
import {
  fmt, fmtFull, fmtPct, toMonthly, freqSuffix, getEffectiveBudget, fmtDate, fmtDateShort,
  FREQ_LABEL, FREQ_OPTIONS,
  computeAssetBuckets, computeTaxBuckets, computeBalanceFinnInsight,
  InfoTooltip, ASSET_CATEGORIES, inputStyle, selectStyle, btnPrimaryStyle, btnSecondaryStyle,
  sectionHeadStyle, MONTH_NAMES, categoryOf, EXPENSE_CATEGORIES, StatementImportPanel,
...
// Estate planning tab — loaded on demand (estate tab only).
const EstatePlanningTab = dynamic(() => import("./estate-planning-tab"), {
  ssr: false,
  loading: () => <ChartSkeleton height={400} />,
});

// Balance sheet tab — loaded on demand (balance tab only).
const BalanceSheetTab = dynamic(() => import("./balance-sheet-tab"), {
  ssr: false,
  loading: () => <ChartSkeleton height={400} />,
});

// Cash flow tab — loaded on demand (cashflow tab only).
const CashFlowTab = dynamic(() => import("./cash-flow-tab"), {
  ssr: false,
  loading: () => <ChartSkeleton height={400} />,
});
...
type Props = {
  ...
  futureEvents: FutureEvent[];          // committed only — feeds the forecast
  allFutureEvents?: FutureEvent[];      // committed + considering — for the Life Plan UI
  homeScenarios: HomeScenario[];
  careerScenarios: CareerScenario[];
  educationScenarios: EducationScenario[];
  familyScenarios: FamilyScenario[];
...
export default function PlanningClient({
  ...
  const retirementYear = profile?.current_age != null && profile?.target_retirement_age != null
    ? currentYear + Math.max(0, profile.target_retirement_age - profile.current_age)
    : null;
  ...
  return (
    <div className="bt-mobile-nav-pad" style={{ padding: "24px", maxWidth: "1280px", width: "100%", margin: "0 auto" }}>
      ...
      {/* ── Tab: My Life Plan ── */}
      {tab === "events" && (
        <div className="bt-mobile-nav-pad" style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
          <LifePlanTab
            readinessScore={lifePlan.futureReadinessScore}
            projectedNWAtRetirement={lifePlan.projectedNWAtRetirement ?? null}
            retirementProb={retirementProb ?? null}
            biggestDecision={lifePlan.biggestDecisions[0] ?? null}
            conflictAlerts={lifePlan.conflictAlerts}
            events={allFutureEvents ?? futureEvents}
            currentYear={currentYear}
