...
export default async function CarPlanningPage() {
  ...
  const [
    ...
    supabase
      ...
      .select("value, is_liability, category, name")
      .eq("user_id", user.id),
    supabase
      .from("cash_flow_items")
      .select("monthly_amount, is_income, category, name")
      .eq("user_id", user.id),
  ]);

  const scenarios: CarScenario[] = (scenariosData ?? []) as CarScenario[];
  ...
  // Pre-fill from balance sheet — vehicle asset for current car value
  const CAR_CATEGORIES = new Set(["vehicle", "car", "auto", "automobile"]);
  const vehicleAsset = (balanceItems ?? []).find(
    (i) => !i.is_liability && CAR_CATEGORIES.has((i.category ?? "").toLowerCase())
  );
  const currentCarValue = vehicleAsset ? Number(vehicleAsset.value) : 0;

  // Pre-fill car loan from liabilities
  const carLoanItem = (balanceItems ?? []).find(
    (i) => i.is_liability && CAR_CATEGORIES.has((i.category ?? "").toLowerCase())
  );
  const currentLoanBalance = carLoanItem ? Number(carLoanItem.value) : 0;

  // Pre-fill car payment from cash flow
  const CAR_CF_CATS = new Set(["car", "auto", "vehicle", "car payment", "auto loan", "car insurance"]);
  const carPaymentItem = (cashFlowItems ?? []).find(
    (i) => !i.is_income && CAR_CF_CATS.has((i.category ?? "").toLowerCase())
  );
  const currentMonthlyPayment = carPaymentItem ? Number(carPaymentItem.monthly_amount) : 0;

  // Liquid assets for down payment suggestion
  ...
  const effectiveIncome = profile?.net_monthly_override
    ?? profile?.gross_monthly_income
    ?? (cashFlowItems ?? []).filter((i) => i.is_income).reduce((s, i) => s + Number(i.monthly_amount), 0)
    ?? 0;

  const effectiveExpenses = profile?.monthly_expenses
    ?? (cashFlowItems ?? []).filter((i) => !i.is_income).reduce((s, i) => s + Number(i.monthly_amount), 0)
    ?? 0;
    ...
    <div style={{ display: "flex", minHeight: "100vh", background: "var(--bg-base)" }}>
      ...
      <div className="bt-main-col" style={{ flex: 1, overflow: "hidden", display: "flex", flexDirection: "column" }}>
        ...
        <CarClient
          ...
          prefillCurrentValue={currentCarValue}
          prefillLoanBalance={currentLoanBalance}
          prefillMonthlyPayment={currentMonthlyPayment}
        />
      </div>
    </div>
  );
