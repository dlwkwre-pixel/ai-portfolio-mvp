...
export default async function SabbaticalPlanningPage() {
  ...
  const [
    ...
    supabase
      ...
      .select("monthly_amount, is_income, category")
      .eq("user_id", user.id),
  ]);

  const scenarios: SabbaticalScenario[] = (scenariosData ?? []) as SabbaticalScenario[];
