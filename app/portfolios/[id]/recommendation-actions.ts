...
async function callGrokForRecommendations(context: unknown, contextNote?: string): Promise<AiRunResponse> {
  ...
  const systemPrompt = [
    ...
    // ── Momentum Protection (gain-locking after parabolic runs)
    "MOMENTUM PROTECTION: When a position has had a significant run (>20% appreciation in under 90 days), you MUST actively evaluate stagnation signals before defaulting to HOLD. Do NOT assume thesis-intact = hold forever through a parabolic peak. Stagnation signals to check via live search and price data: (a) price flat or declining relative to the broader market for 2+ consecutive weeks after the run; (b) volume contracting on any continued upside; (c) position approaching or above a round-number resistance, prior high, or analyst price target range. If 2 or more stagnation signals are present after a >20% run: recommend a PROTECT_GAINS trim action (20–50% of the position size) with an explicit re-entry target. This is NOT a thesis change — label it clearly as 'gain protection trim, thesis intact' and state the re-entry condition (e.g., 'trim here, re-enter on a pullback to $X or on next confirmed catalyst'). When macro backdrop carries elevated geopolitical or event risk (active military conflicts, major central bank decisions, earnings season uncertainty), lower the stagnation threshold to a >15% run. NEVER hold into obvious stagnation after a parabolic move without at minimum flagging the momentum risk explicitly in the rationale — passive HOLD after a big run is not neutral, it is a specific decision that requires justification.",
