...
async function callGrokForRecommendations(context: unknown, contextNote?: string): Promise<AiRunResponse> {
  ...
  const systemPrompt = [
    ...
    // ── Data Discipline
    "DATA: Current prices, Finnhub data, and Reddit sentiment are pre-loaded in context. DO NOT use training-data memory for stock prices. Use web_search or x_search ONLY for: (1) prices of NEW buy candidates not in holdings, (2) very recent catalysts not in the Finnhub feed, (3) prices showing $0 or null. Max 2-3 searches per run.",
...
- OPPORTUNITY DISCOVERY (ENGINE 2): Run this engine independently. Scan for external candidates competing for available capital. Exploration breadth is determined by portfolio conditions — idle cash, concentration level (HHI), strategy aggressiveness, environment quality, and conviction gaps — NOT by holding count. A 2-stock concentrated portfolio with 35% cash demands active external scanning. Surface genuine candidates where conviction is earned; never cap exploration by how many holdings already exist.
