...
export default function RunAiControls({
  ...
  return (
    <div className="space-y-3">
      ...
      {/* Health report — score gauge + structured insight tiles */}
      ...
      <div style={{ display: "flex", alignItems: "center", gap: "16px", padding: "16px 18px", borderBottom: "1px solid var(--border-subtle)" }}>
        ...
        <div style={{ minWidth: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
            ...
            <span style={{ fontSize: "10px", fontWeight: 700, color: accent, background: score >= 70 ? "var(--green-bg)" : score >= 50 ? "var(--amber-bg)" : "var(--red-bg)", border: `1px solid ${score >= 70 ? "var(--green-border)" : score >= 50 ? "var(--amber-border)" : "var(--red-border)"}`, padding: "1px 8px", borderRadius: "var(--radius-full)" }}>{verdict}</span>
          </div>
          {report.headline && (
            <p style={{ fontSize: "14px", fontWeight: 600, color: "var(--text-primary)", lineHeight: 1.4, margin: "4px 0 0" }}>{report.headline}</p>
          )}
          <p style={{ fontSize: "10px", color: "var(--text-muted)", margin: "3px 0 0" }}>Cross-checked by Gemini · educational, not advice</p>
        </div>
      </div>
